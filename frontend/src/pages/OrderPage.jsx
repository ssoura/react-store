...
const OrderPage = () => {
  ...
  const orderPay = useSelector((state) => state.order.pay);
  const { loading: loadingPay, success: successPay } = orderPay;

  const orderDeliver = useSelector((state) => state.order.deliver);
  ...
  useEffect(() => {
    ...
    const addPayPalScript = async () => {
      const { data: clientId } = await axios.get(
        `${import.meta.env.VITE_APP_API}/api/config/paypal`
      );
